...
발송 엔드포인트는 **요청 단위**로 카운트합니다. 수신자 50명인 요청 하나는 `per_minute` 에 1건, `daily` · `monthly` 에 50건으로 반영됩니다.
...
한 요청에 최대 50명까지 수용됩니다. 수신자별로 각각 호출하지 말고 하나의 요청에 묶되, 50명을 초과하는 발송은 여러 번의 50명 단위 호출로 분할하세요.
